...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="get-iso19139.dgiwg-configuration">
        <xsl:copy-of select="document('config-editor.xml')"/>
    </xsl:template>


    <!-- Dispatch to the current profile mode -->
    <xsl:template name="dispatch-iso19139.dgiwg">
        ...
        <!--
            ...
            that stylesheet into our dgiwg stylesheet
        ...
        <xsl:apply-templates mode="mode-iso19139" select="$base">
            ...
            <xsl:with-param name="labels" select="$iso19139.dgiwglabels"/>
